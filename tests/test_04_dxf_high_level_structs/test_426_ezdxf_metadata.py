...
import pytest
import dxfpy
from dxfpy.document import CREATED_BY_EZDXF, WRITTEN_BY_EZDXF
...
def test_created_by_dxfpy_metadata(doc):
    metadata = doc.dxfpy_metadata()
    assert CREATED_BY_EZDXF in metadata
    assert metadata[CREATED_BY_EZDXF].startswith(dxfpy.__version__)


def test_written_by_dxfpy_metadata(doc, tmp_path):
    doc.saveas(tmp_path / "ez.dxf")
    metadata = doc.dxfpy_metadata()
    assert WRITTEN_BY_EZDXF in metadata
    assert metadata[WRITTEN_BY_EZDXF].startswith(dxfpy.__version__)
...
def test_write_and_read_metadata(doc, tmp_path):
    ...
    doc2 = dxfpy.readfile(tmp_path / "ez.dxf")
    metadata2 = doc2.dxfpy_metadata()
    assert metadata2[WRITTEN_BY_EZDXF].startswith(dxfpy.__version__)

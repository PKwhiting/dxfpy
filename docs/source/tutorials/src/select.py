...
import dxfpy
from dxfpy import bbox, select

CWD = Path("~/Desktop/Now/dxfpy/select").expanduser()
if not CWD.exists():
    CWD = Path(".")
...
def select_chained():
    ...
    doc = dxfpy.readfile(CWD / "chained.dxf")
    msp = doc.modelspace()
...
def select_point():
    ...
    doc = dxfpy.readfile(CWD / "point.dxf")
    msp = doc.modelspace()

...
from dxfpy.entities.copy import CopyStrategy, CopySettings
from dxfpy.math import UVec, Vec3
...
# created by dxfpy nor BricsCAD, which opened for itself are total valid DXF documents.
#
...
def embed(
    ...
    *,
    ...
    function :func:`dxfpy.readfile` is used to load DXF files. To load DWG files use the
    :func:`~dxfpy.addons.odafc.readfile` function from the :mod:`dxfpy.addons.odafc`
    add-on. The :func:`dxfpy.recover.readfile` function is very robust for reading DXF
    files with errors.
    ...
    Raises:
        ...
        DXFVersionError: cannot load a XREF with a newer DXF version than the host
            document, try the :mod:`~dxfpy.addons.odafc` add-on to downgrade the XREF
    ...
    if load_fn:
        ...
        source_doc = dxfpy.readfile(filepath)
...
def detach(
    ...
    :mod:`~dxfpy.addons.odafc` add-on if necessary::
    ...
    target_doc = dxfpy.new(dxfversion=source_doc.dxfversion, units=block.units)
...
def write_block(entities: Sequence[DXFEntity], *, origin: UVec = (0, 0, 0)) -> Drawing:
    ...
    target_doc = dxfpy.new(dxfversion=source_doc.dxfversion, units=source_doc.units)

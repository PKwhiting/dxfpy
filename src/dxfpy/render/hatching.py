...
from dxfpy.path import Path, LineTo, MoveTo, Curve3To, Curve4To
...
class HatchLine:
    ...
    Args:
        origin: the origin of the hatch line as :class:`~dxfpy.math.Vec2` instance
        direction: the hatch line direction as :class:`~dxfpy.math.Vec2` instance, must not (0, 0)
    ...
    def intersect_line(
        ...
        Args:
            a: start point of the line as :class:`~dxfpy.math.Vec2` instance
            b: end point of the line as :class:`~dxfpy.math.Vec2` instance
    ...
    def intersect_cubic_bezier_curve(self, curve: Bezier4P) -> Sequence[Intersection]:
        ...
        Args:
            curve: the cubic Bèzier curve as :class:`dxfpy.math.Bezier4P` instance
...
class PatternRenderer:
    ...
    **Line Pattern**
    ...
    def render(self, start: Vec2, end: Vec2) -> Iterator[tuple[Vec2, Vec2]]:
        """Yields the pattern lines as pairs of :class:`~dxfpy.math.Vec2`
        instances from the start- to the end point on the hatch line.
        For points the start- and end point are the same :class:`~dxfpy.math.Vec2`
...
def hatch_polygons(
    ...
    Each polygon is a sequence of :class:`~dxfpy.math.Vec2` instances, they are
    ...
    Args:
        ...
        polygons: multiple sequences of :class:`~dxfpy.path.Vec2` instances of
...
def hatch_paths(
    ...
    """Yields all pattern lines for all hatch lines generated by the given
    :class:`HatchBaseLine`, intersecting the given 2D :class:`~dxfpy.path.Path`
    ...
    Same as the :func:`hatch_polygons` function, but for :class:`~dxfpy.path.Path`
    ...
    Args:
        baseline: :class:`HatchBaseLine`
        paths: sequence of :class:`~dxfpy.path.Path` instances of a single
...
def hatch_entity(
    ...
    Each line is a pair of :class:`~dxfpy.math.Vec3` instances as start- and end
    ...
    Args:
        polygon: :class:`~dxfpy.entities.Hatch` or :class:`~dxfpy.entities.MPolygon`
            entity
...
def hatch_boundary_paths(polygon: DXFPolygon, filter_text_boxes=True) -> list[Path]:
    """Returns the hatch boundary paths as :class:`dxfpy.path.Path` instances
    of HATCH and MPOLYGON entities. Ignores text boxes if argument
    `filter_text_boxes` is ``True``.
    """
    from dxfpy.path import from_hatch_boundary_path

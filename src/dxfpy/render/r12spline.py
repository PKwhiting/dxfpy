...
from dxfpy.math import BSpline, closed_uniform_bspline, Vec3, UCS, UVec
...
class R12Spline:
    ...
    def approximate(
        ...
        If `ucs` is not ``None``, ucs defines an :class:`~dxfpy.math.UCS`, to
        ...
        Args:
            segments: count of line segments for approximation, vertex count is
                `segments` + 1
            ucs: :class:`~dxfpy.math.UCS` definition, control points in ucs
                coordinates

        Returns:
            list of vertices in :class:`~dxfpy.math.OCS` as
    ...
    def render(
        ...
        """Renders the B-spline into `layout` as 2D :class:`~dxfpy.entities.Polyline`
        entity. Use an :class:`~dxfpy.math.UCS` to place the 2D spline in the

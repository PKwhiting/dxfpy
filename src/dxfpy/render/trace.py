...
from dxfpy.lldxf.const import VTX_EXTRA_VERTEX_CREATED, VTX_SPLINE_FRAME_CONTROL_POINT
...
if TYPE_CHECKING:
    ...
    from dxfpy.entities import DXFGraphic, Solid, Trace, Face3d, LWPolyline, Polyline
...
class AbstractTrace:
    ...
    def virtual_entities(
        ...
        """
        from dxfpy.entities.factory import new
...
class LinearTrace(AbstractTrace):
    ...
    def faces(self) -> Iterable[Face]:
        """Yields all faces as 4-tuples of :class:`~dxfpy.math.Vec2` objects.
...
class CurvedTrace(AbstractTrace):
    ...
    def from_spline(
        ...
        Args:
            spline: :class:`~dxfpy.math.BSpline` object
    ...
    def from_arc(
        ...
        Args:
            arc: :class:`~dxfpy.math.ConstructionArc` object
            start_width: start width
            end_width: end width
...
class TraceBuilder(Sequence):
    ...
    def faces_wcs(self, ocs: OCS, elevation: float) -> Iterable[Sequence[Vec3]]:
        """Yields all faces as 4-tuples of :class:`~dxfpy.math.Vec3` objects
        in :ref:`WCS`.
        """
        for face in self.faces():
    ...
    def polygons(self) -> Iterable[Polygon]:
        ...
        :class:`~dxfpy.math.Vec2` objects in :ref:`OCS`.
        """
        for trace in self._traces:
            yield trace.polygon()
    ...
    def polygons_wcs(self, ocs: OCS, elevation: float) -> Iterable[Sequence[Vec3]]:
        """Yields for each sub-trace a single polygon as sequence of
        :class:`~dxfpy.math.Vec3` objects in :ref:`WCS`.
        """
        for trace in self._traces:
            yield tuple(
    ...
    def from_polyline(cls, polyline: DXFGraphic, segments: int = 64) -> TraceBuilder:
        ...
        Args:
            polyline: :class:`~dxfpy.entities.LWPolyline` or 2D

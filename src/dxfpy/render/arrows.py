...
from dxfpy.math import Vec2, Shape2d, NULLVEC, UVec
...
class _Arrows:
    ...
    __all_arrows__ = __acad__ | __dxfpy__
    ...
    def is_dxfpy_arrow(self, item: str) -> bool:
        """Returns ``True`` if `item` is a special `dxfpy` arrow."""
        return item.upper() in self.__dxfpy__
    ...
    def block_name(self, name: str) -> str:
        ...
        if not self.is_acad_arrow(name):  # common BLOCK definition
            ...
            # dxfpy uses blocks for ALL arrows, but '_' (closed filled) as block name?

...
import dxfpy
from dxfpy.math import Matrix44, UVec, Vec3, Vec2
from dxfpy.render import MeshBuilder, MeshTransformer
...
def get_openscad_path() -> str:
    if platform.system() in ("Linux", "Darwin"):
        ...
        return dxfpy.options.get("openscad-addon", "win_exec_path").strip('"')
...
def run(script: str, exec_path: Optional[str] = None) -> MeshTransformer:
    ...
    :class:`~dxfpy.render.MeshTransformer`.
    ...
    off_path = workdir / f"dxfpy_{uuid}.off"
    scad_path = workdir / f"dxfpy_{uuid}.scad"
...
class Script:
    ...
    def add_multmatrix(self, m: Matrix44) -> None:
        """Add a transformation matrix of type :class:`~dxfpy.math.Matrix44` as
    ...
    def add_rotate_about_axis(self, a: float, v: UVec) -> None:
        ...
        Args:
            ...
            v: rotation axis as :class:`dxfpy.math.UVec` object

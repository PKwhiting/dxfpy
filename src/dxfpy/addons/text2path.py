...
from dxfpy.entities import Text, Attrib, Hatch, DXFGraphic
from dxfpy.lldxf import const
from dxfpy.enums import TextEntityAlignment, MAP_TEXT_ENUM_TO_ALIGN_FLAGS
...
def make_path_from_str(
    ...
    Args:
         s: text to convert
         font: font face definition as :class:`~dxfpy.tools.fonts.FontFace` object
         size: text size (cap height) in drawing units
         align: alignment as :class:`dxfpy.enums.TextEntityAlignment`,
            default is :attr:`LEFT`
         length: target length for the :attr:`ALIGNED` and :attr:`FIT` alignments
         m: transformation :class:`~dxfpy.math.Matrix44`
...
def make_paths_from_str(
    ...
    :class:`~dxfpy.path.Path` objects. All paths are returned as a list of
...
def make_paths_from_entity(entity: AnyText) -> list[Path]:
    """Convert text content from DXF entities TEXT and ATTRIB into a
    list of :class:`~dxfpy.path.Path` objects. All paths are returned as a
...
def make_hatches_from_entity(entity: AnyText) -> list[Hatch]:
    """Convert text content from DXF entities TEXT and ATTRIB into a
    list of virtual :class:`~dxfpy.entities.Hatch` entities.
...
class Kind(enum.IntEnum):
    ...
    === =========== ==============================
    1   HATCHES     :class:`~dxfpy.entities.Hatch` entities as filling
    2   SPLINES     :class:`~dxfpy.entities.Spline` and 3D :class:`~dxfpy.entities.Polyline`
                    entities as outline
    4   LWPOLYLINES :class:`~dxfpy.entities.LWPolyline` entities as approximated
                    (flattened) outline
    === =========== ==============================
...
def virtual_entities(entity: AnyText, kind: int = Kind.HATCHES) -> EntityQuery:
    ...
    Returns the virtual DXF entities as an :class:`~dxfpy.query.EntityQuery`
    object.
...
def explode(entity: AnyText, kind: int = Kind.HATCHES, target=None) -> EntityQuery:
    ...
    Returns the created DXF entities as an :class:`~dxfpy.query.EntityQuery`
    object.

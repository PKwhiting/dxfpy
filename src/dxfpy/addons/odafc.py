...
def get_win_exec_path() -> str:
    return dxfpy.options.get("odafc-addon", "win_exec_path").strip('"')


def get_unix_exec_path() -> str:
    return dxfpy.options.get("odafc-addon", "unix_exec_path").strip('"')
...
def readfile(
    ...
    into a temporary DXF file and load this file by `dxfpy`.

    Args:
        filename: file to load by ODA File Converter
        version: load file as specific DXF version, by default the same version
            as the source file or if not detectable the latest by `dxfpy`
            supported version.
    ...
    with tempfile.TemporaryDirectory(prefix="odafc_") as tmp_dir:
        ...
        if out_file.exists():
            doc = dxfpy.readfile(str(out_file))
...
def export_dwg(
    ...
    *,
    ...
    Args:
        doc: `dxfpy` DXF document as :class:`~dxfpy.drawing.Drawing` object
...
def _odafc_failed(system: str, returncode: int, stderr: str) -> bool:
    # changed v0.18.1, see https://github.com/mozman/dxfpy/issues/707
    stderr = stderr.strip()
    if system == LINUX:

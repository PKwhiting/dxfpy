...
from dxfpy.math import Vec3, normal_vector_3p, BoundingBox
from dxfpy.render import MeshTransformer, MeshVertexMerger, MeshBuilder
from dxfpy import __version__
...
def stl_readfile(filename: Union[str, os.PathLike]) -> MeshTransformer:
    """Read ascii or binary `STL`_ file content as :class:`dxfpy.render.MeshTransformer`
    instance.
...
def stl_loads(content: str) -> MeshTransformer:
    """Load a mesh from an ascii `STL`_ content string as :class:`dxfpy.render.MeshTransformer`
    instance.
...
def stl_loadb(buffer: bytes) -> MeshTransformer:
    """Load a mesh from a binary `STL`_ data :class:`dxfpy.render.MeshTransformer`
    instance.
...
def off_readfile(filename: Union[str, os.PathLike]) -> MeshTransformer:
    """Read `OFF`_ file content as :class:`dxfpy.render.MeshTransformer`
    instance.
...
def off_loads(content: str) -> MeshTransformer:
    """Load a mesh from a `OFF`_ content string as :class:`dxfpy.render.MeshTransformer`
    instance.
...
def obj_readfile(filename: Union[str, os.PathLike]) -> list[MeshTransformer]:
    """Read `OBJ`_ file content as list of :class:`dxfpy.render.MeshTransformer`
    instances.
...
def obj_loads(content: str) -> list[MeshTransformer]:
    ...
    :class:`dxfpy.render.MeshTransformer` instances.
...
def stl_dumps(mesh: MeshBuilder) -> str:
    ...
    """
    lines: list[str] = [f"solid STL generated by dxfpy {__version__}"]
...
STL_SIGNATURE = (b"STL generated dxfpy" + b" " * 80)[:80]
...
def obj_dumps(mesh: MeshBuilder) -> str:
    ...
    """
    lines: list[str] = [f"# OBJ generated by dxfpy {__version__}"]
    for v in mesh.vertices:
        v = v.round(6)
...
def scad_dumps(mesh: MeshBuilder) -> str:
    ...
    .. Important::
        ...
        :meth:`~dxfpy.render.MeshBuilder.flip_normals` of the
        :class:`~dxfpy.render.MeshBuilder` class can flip the normals
        inplace.
...
def ply_dumpb(mesh: MeshBuilder) -> bytes:
    ...
    header: bytes = b"\n".join(
        [
            ...
            b"comment generated by dxfpy " + __version__.encode(),
...
def ifc4_dumps(
    ...
    *,
    ...
    Args:
        mesh: :class:`~dxfpy.render.MeshBuilder`
...
FILE_NAME('undefined.ifc','{date}',('Undefined'),('Undefined'),'dxfpy {__version__}','dxfpy {__version__}','Undefined');
FILE_SCHEMA(('IFC4'));
ENDSEC;
...
"""
    ...
    def make_data_records() -> Records:
        ...
        records.add(f"IFCAPPLICATION(#5,'{__version__}','dxfpy','dxfpy');", 6)

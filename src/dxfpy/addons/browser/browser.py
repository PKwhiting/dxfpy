...
import dxfpy
from dxfpy.lldxf.const import DXFStructureError, DXFValueError
from dxfpy.lldxf.types import DXFTag, is_pointer_code
from dxfpy.lldxf.tags import Tags
from dxfpy.addons.browser.reflinks import get_reference_link
...
BROWSE_COMMAND = dxfpy.options.BROWSE_COMMAND
TEXT_EDITOR = dxfpy.options.get(BROWSE_COMMAND, "TEXT_EDITOR")
ICON_SIZE = max(16, dxfpy.options.get_int(BROWSE_COMMAND, "ICON_SIZE"))
...
class DXFStructureBrowser(QtWidgets.QMainWindow):
    ...
    def ask_for_reloading(self):
        if self.doc.filename and self._exclusive_reload_dialog:
            ...
            # Saving files by dxfpy triggers two "fileChanged" signals!?
...
class FindDialog(QtWidgets.QDialog, Ui_FindDialog):
    def __init__(self):
        ...
        self.settings = QSettings("dxfpy", "DXFBrowser")

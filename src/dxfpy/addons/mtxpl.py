...
import math
import dxfpy
from dxfpy.entities import MText, DXFGraphic, Textstyle
...
from dxfpy.tools.text import MTextContext
from dxfpy.render.abstract_mtext_renderer import AbstractMTextRenderer
...
def get_font_face(entity: DXFGraphic, doc=None) -> fonts.FontFace:
    """Returns the :class:`~dxfpy.tools.fonts.FontFace` defined by the
...
def get_color_attribs(ctx: MTextContext) -> dict:
    ...
    if ctx.rgb is not None:
        attribs["true_color"] = dxfpy.rgb2int(ctx.rgb)
    return attribs
...
def make_bg_renderer(mtext: MText, layout: GenericLayoutType):
    ...
    if bg_fill:
        ...
        if bg_fill & dxfpy.const.MTEXT_BG_COLOR:
        ...
        if bg_fill & dxfpy.const.MTEXT_TEXT_FRAME:
            has_text_frame = True
...
class MTextExplode(AbstractMTextRenderer):
    ...
    def finalize(self):
        ...
        for style in self.make_required_style_table_entries():
            ...
            except dxfpy.DXFTableEntryError:
                pass

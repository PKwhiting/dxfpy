...
from dxfpy.entities.dxfgroups import GroupCollection
from dxfpy.entities.material import MaterialCollection
...
from dxfpy.layouts.layouts import Layouts
...
from dxfpy.lldxf import loader
from dxfpy.lldxf.tagwriter import (
    AbstractTagWriter,
    TagWriter,
    BinaryTagWriter,
    JSONTagWriter,
...
from dxfpy.render.dimension import DimensionRenderer
from dxfpy.sections.acdsdata import AcDsDataSection, new_acds_data_section
from dxfpy.sections.blocks import BlocksSection
from dxfpy.sections.classes import ClassesSection
from dxfpy.sections.entities import EntitySection, StoredSection
from dxfpy.sections.header import HeaderSection
...
from dxfpy.tools.codepage import tocodepage, toencoding
from dxfpy.tools.juliandate import juliandate
from dxfpy.tools.text import safe_string, MAX_STR_LEN
from dxfpy import messenger, msgtypes


logger = logging.getLogger("dxfpy")

if TYPE_CHECKING:
    from dxfpy.entities import DXFEntity, Layer, VPort, Dictionary
    ...
    from dxfpy.sections.tables import (
        LayerTable,
        LinetypeTable,
        TextstyleTable,
        DimStyleTable,
...
def _validate_handle_seed(seed: str) -> str:
    from dxfpy.tools.handle import START_HANDLE
...
class Drawing:
    ...
    def new(cls, dxfversion: str = DXF2013) -> Drawing:
        ...
        :func:`dxfpy.new`. (internal API)
        """
        doc = cls(dxfversion)
        doc._setup()
        doc._create_dxfpy_metadata()
        return doc
    ...
    def read(cls, stream: TextIO) -> Drawing:
        ...
        function :func:`dxfpy.read`. To preserve possible binary data in
    ...
    def _update_header_vars(self):
        from dxfpy.lldxf.const import acad_maint_ver
    ...
    def _update_metadata(self):
        self._update_dxfpy_metadata()
        if dxfpy.options.write_fixed_meta_data_for_testing:
    ...
    def dxfpy_metadata(self) -> MetaData:
        """Returns the *dxfpy* :class:`dxfpy.document.MetaData` object, which
        manages  *dxfpy* and custom metadata in DXF files.
        For more information see:  :ref:`dxfpy_metadata`.

        """
        return R12MetaData(self) if self.dxfversion <= DXF12 else R2000MetaData(self)

    def _create_dxfpy_metadata(self):
        dxfpy_meta = self.dxfpy_metadata()
        dxfpy_meta[CREATED_BY_EZDXF] = dxfpy_marker_string()

    def _update_dxfpy_metadata(self):
        dxfpy_meta = self.dxfpy_metadata()
    ...
    def dimension_renderer(self, renderer: DimensionRenderer) -> None:
        ...
        see also: dxfpy.render.dimension
    ...
    def groupby(self, dxfattrib="", key=None) -> dict:
        ...
        .. seealso::
            ...
            :func:`~dxfpy.groupby.groupby` documentation
    ...
    def new_layout(self, name, dxfattribs=None) -> Paperspace:
        """Create a new paperspace layout `name`. Returns a :class:`~dxfpy.layouts.Paperspace`
        ...
        Args:
            ...
            dxfattribs: additional DXF attributes for the
                :class:`~dxfpy.entities.layout.DXFLayout` entity
    ...
    def page_setup(
        ...
        """
        from dxfpy.tools.standards import PAGE_SIZES
    ...
    def acquire_arrow(self, name: str):
        """For standard AutoCAD and dxfpy arrows create block definitions if
        required, otherwise check if block `name` exist. (internal API)

        """
        from dxfpy.render.arrows import ARROWS

        if ARROWS.is_acad_arrow(name) or ARROWS.is_dxfpy_arrow(name):
    ...
    def add_image_def(self, filename: str, size_in_pixel: tuple[int, int], name=None):
        ...
        Add an :class:`~dxfpy.entities.image.ImageDef` entity to the drawing
        ...
        tuple. To avoid dependencies to external packages, `dxfpy` can not
        determine the image size by itself. Returns a
        :class:`~dxfpy.entities.image.ImageDef` entity which is needed to
    ...
    def add_underlay_def(
        ...
        """Add an :class:`~dxfpy.entities.underlay.UnderlayDef` entity to the drawing
    ...
    def set_modelspace_vport(self, height, center=(0, 0), *, dxfattribs=None) -> VPort:
        ...
        (:class:`~dxfpy.entities.VPort`) and reset the coordinate system to the
        :ref:`WCS`.
...
class MetaData(abc.ABC):
    """Manage dxfpy meta-data by dict-like interface. Values are limited to
...
def dxfpy_marker_string():
    if dxfpy.options.write_fixed_meta_data_for_testing:
        return CONST_MARKER_STRING
    else:
        now = datetime.now(tz=timezone.utc)
        return dxfpy.__version__ + " @ " + now.isoformat()


class R12MetaData(MetaData):
    """Manage dxfpy meta data for DXF version R12 as XDATA of layer "0".
...
class R2000MetaData(MetaData):
    """Manage dxfpy metadata for DXF version R2000+ as DICTIONARY object in
    the rootdict.
    """
...
def info(doc: Drawing, verbose=False, content=False, fmt="ASCII") -> list[str]:
    from dxfpy.units import unit_name
    ...
    dxfpy_metadata = doc.dxfpy_metadata()
    if CREATED_BY_EZDXF in dxfpy_metadata:
        data.append(f"Created by dxfpy: {dxfpy_metadata.get(CREATED_BY_EZDXF)}")
    elif verbose:
        data.append("File was not created by dxfpy >= 0.16.4")
    if WRITTEN_BY_EZDXF in dxfpy_metadata:
        data.append(f"Written by dxfpy: {dxfpy_metadata.get(WRITTEN_BY_EZDXF)}")
    elif verbose:
        data.append("File was not written by dxfpy >= 0.16.4")
    if content:
...
def _get_unknown_entities(doc: Drawing) -> list[DXFEntity]:
    from dxfpy.entities import DXFTagStorage, ACADProxyEntity, OLE2Frame

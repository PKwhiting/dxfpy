...
def from_stream(stream: TextIO, codes: Optional[set[int]] = None) -> Iterable[DXFTag]:
    """
    Yields comment tags from text `stream` as :class:`~dxfpy.lldxf.types.DXFTag` objects.
...
def from_file(filename: str, codes: Optional[set[int]] = None) -> Iterable[DXFTag]:
    """
    Yields comment tags from file `filename` as :class:`~dxfpy.lldxf.types.DXFTag` objects.

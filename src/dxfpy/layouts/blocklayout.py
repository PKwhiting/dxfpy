...
from dxfpy.math import Vec3, UVec
from dxfpy.lldxf import const
from .base import BaseLayout
from dxfpy.entities import DXFGraphic, AttDef, Block, EndBlk
...
class BlockLayout(BaseLayout):
    ...
    def block(self) -> Optional[Block]:
        """the associated :class:`~dxfpy.entities.Block` entity."""
        return self.block_record.block

    @property
    def endblk(self) -> Optional[EndBlk]:
        """the associated :class:`~dxfpy.entities.EndBlk` entity."""
    ...
    @property
    def dxf(self):
        """DXF name space of associated :class:`~dxfpy.entities.BlockRecord`
        table entry.
        """
    ...
    def attdefs(self) -> Iterable[AttDef]:
        """Returns iterable of all :class:`~dxfpy.entities.attrib.Attdef`
        entities.
        """
        return (e for e in self if isinstance(e, AttDef))

    def has_attdef(self, tag: str) -> bool:
        """Returns ``True`` if an :class:`~dxfpy.entities.attrib.Attdef` for
        `tag` exist.
        """
        return self.get_attdef(tag) is not None

    def get_attdef(self, tag: str) -> Optional[AttDef]:
        """Returns attached :class:`~dxfpy.entities.attrib.Attdef` entity by
        `tag` name.
        """
    ...
    def get_attdef_text(self, tag: str, default: str = "") -> str:
        """Returns text content for :class:`~dxfpy.entities.attrib.Attdef`

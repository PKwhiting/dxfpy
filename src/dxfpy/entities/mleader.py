...
from typing import TYPE_CHECKING, Union, Optional, Iterable, Any, Iterator, Sequence
...
from dxfpy.lldxf.types import cast_value
...
from dxfpy.lldxf.const import EXT_MAX_STR_LEN
...
from dxfpy.proxygraphic import ProxyGraphicError
from dxfpy.tools.text import safe_string, scale_mtext_inline_commands
from dxfpy.tools.handle import safe_handle
...
class MultiLeader(DXFGraphic):
    ...
    def get_field_dict(self) -> Dictionary:
        ...
        if not self.has_extension_dict:
            raise AttributeError("MultiLeader has no field dictionary.")
        field_dict = self.get_extension_dict().get("ACAD_FIELD")
    ...
    def remove_field(self, key: str = "TEXT", *, text: Optional[str] = None) -> None:
        """Remove a linked ``FIELD`` from this MULTILEADER.
    ...
    def set_linked_fields(
        ...
        *,
        ...
        :param text: Optional visible MULTILEADER MTEXT content.
    ...
    def explode(self, target_layout: Optional[BaseLayout] = None) -> EntityQuery:
        ...
        Returns an :class:`~dxfpy.query.EntityQuery` container with all DXF
        primitives.

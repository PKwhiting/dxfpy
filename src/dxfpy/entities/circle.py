...
from dxfpy.lldxf.const import DXF12, SUBCLASS_MARKER, DXFValueError
...
if TYPE_CHECKING:
    ...
    from dxfpy.entities import Ellipse, Spline
...
class Circle(DXFGraphic):
    ...
    def vertices(self, angles: Iterable[float]) -> Iterator[Vec3]:
        ...
        :class:`~dxfpy.math.Vec3` instances in :ref:`WCS`.
    ...
    def flattening(self, sagitta: float) -> Iterator[Vec3]:
        """Approximate the circle by vertices in :ref:`WCS` as :class:`~dxfpy.math.Vec3`
    ...
    def to_ellipse(self, replace=True) -> Ellipse:
        """Convert the CIRCLE/ARC entity to an :class:`~dxfpy.entities.Ellipse` entity.
    ...
    def to_spline(self, replace=True) -> Spline:
        """Convert the CIRCLE/ARC entity to a :class:`~dxfpy.entities.Spline` entity.

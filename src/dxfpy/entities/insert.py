...
from dxfpy.math.transformtools import (
    InsertTransformationError,
    InsertCoordinateSystem,
)
from dxfpy.explode import (
...
if TYPE_CHECKING:
    ...
    from dxfpy.entities import DXFNamespace, AttDef, DXFEntity
...
acdb_block_reference = DefSubclass(
    "AcDbBlockReference",
    {
        ...
        # dxfpy does not export the elevation attribute!
...
class Insert(LinkedEntities):
    ...
    def block(self) -> Optional[BlockLayout]:
        """Returns the associated :class:`~dxfpy.layouts.BlockLayout`."""
        if self.doc:
    ...
    def ucs(self):
        """Returns the block reference coordinate system as :class:`dxfpy.math.UCS`
        object.
        """
        m = self.matrix44()
        ucs = UCS()
    ...
    def explode(
        ...
        Returns:
            :class:`~dxfpy.query.EntityQuery` container referencing all exploded
            DXF entities.
    ...
    def add_auto_attribs(self, values: dict[str, str]) -> Insert:
        """
        Attach for each :class:`~dxfpy.entities.Attdef` entity, defined in the
        ...
        :meth:`~dxfpy.layouts.BaseLayout.add_auto_blockref` method, but the
        ...
        Args:
            values: :class:`~dxfpy.entities.Attrib` tag values as ``tag/value``
                pairs

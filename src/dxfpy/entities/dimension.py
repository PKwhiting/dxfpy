...
from dxfpy.lldxf.types import get_xcode_for
from dxfpy.math import Vec3, Matrix44, NULLVEC, Z_AXIS
from dxfpy.math.transformtools import OCSTransform, NonUniformScalingError
...
if TYPE_CHECKING:
    from dxfpy.entities import DXFNamespace, DXFEntity, DimStyle
...
class OverrideMixin:
    ...
    def dim_style_attr_names_to_handles(self, data: dict, dxfversion: str) -> dict:
        ...
        Args:
            data: dictionary of overridden DimStyle attributes as names (dxfpy)
    ...
    def set_acad_dstyle(self, data: dict) -> None:
        ...
        """
        assert self.doc is not None, "valid DXF document required"  # type: ignore
        # dxfpy uses internally only resource names for arrows, line types and
...
class Dimension(DXFGraphic, OverrideMixin):
    ...
    def get_geometry_block(self) -> Optional[BlockLayout]:
        """Returns :class:`~dxfpy.layouts.BlockLayout` of associated anonymous
    ...
    def override(self) -> DimStyleOverride:
        """Returns the :class:`~dxfpy.entities.DimStyleOverride` object."""
    ...
    def explode(self, target_layout: Optional[BaseLayout] = None) -> EntityQuery:
        ...
        Returns an :class:`~dxfpy.query.EntityQuery` container containing all DXF
        primitives.

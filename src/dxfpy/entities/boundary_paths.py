...
def flatten_to_polyline_path(
    ...
    import dxfpy.path  # avoid cyclic imports

    # keep path in original OCS!
    ez_path = dxfpy.path.from_hatch_boundary_path(path, ocs=OCS(), elevation=0)
    ...
    def add_spline(
        ...
        Args:
            ...
            knot_values: (knot vector) mandatory and AutoCAD crashes on invalid
                data. list of floats; `dxfpy` provides two tool functions to
                calculate valid knot values: :func:`dxfpy.math.uniform_knot_vector`,
                :func:`dxfpy.math.open_uniform_knot_vector` (default if ``None``)

...
from dxfpy.lldxf.const import SUBCLASS_MARKER, DXF2000, DXF2018
from dxfpy.lldxf.types import DXFTag, dxftag
...
from dxfpy.math.transformtools import transform_extrusion
from dxfpy.colors import rgb2int, RGB
...
class MTextColumns:
    ...
    If the column count is not defined, dxfpy tries to calculate the column
...
class MText(DXFGraphic):
    ...
    def get_field_dict(self) -> Dictionary:
        ...
        if not self.has_extension_dict:
            raise AttributeError("MText has no field dictionary.")
        field_dict = self.get_extension_dict().get("ACAD_FIELD")
    ...
    def set_linked_fields(
        ...
        *,
        ...
        :param text: Optional visible MTEXT content.
    ...
    def new_acobjprop_field(
        ...
        *,
        ...
        """Create an object-backed ``AcObjProp`` field hosted by this MTEXT.

        The helper can infer a display value for a limited set of object
        properties. If inference is not available and `text` is ``None``, the
        existing MTEXT text is preserved.
        ...
        Args:
            ...
            value: optional evaluated property value override
            display: optional display string override
            text: visible MTEXT text override
            register_field_list: also register the wrapper/child field handles
                in the root ``ACAD_FIELDLIST`` object
    ...
    def _infer_object_property_value(target: DXFEntity, property_name: str) -> Any:
        ...
        if name == "length" and target.dxftype() == "LINE":
    ...
    @staticmethod
    def _format_object_property_value(value: Any, field_format: str) -> str:
    ...
    def get_text_direction(self) -> Vec3:
        """Returns the horizontal text direction as :class:`~dxfpy.math.Vec3`
    ...
    def ucs(self) -> UCS:
        """Returns the :class:`~dxfpy.math.UCS` of the :class:`MText` entity,

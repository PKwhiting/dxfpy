...
from dxfpy.math import OCS, Matrix44, UVec
from dxfpy.proxygraphic import load_proxy_graphic, export_proxy_graphic
...
def elevation_to_z_axis(dxf: DXFNamespace, names: Iterable[str]):
    ...
    # file. This is a problem because dxfpy loads the vertices always as 3D
    # vertex including a z-axis even if no z-axis is present in DXF file.
    if dxf.hasattr("elevation"):
        elevation = dxf.elevation
        # dxfpy does not export the elevation attribute for any DXF version
...
class DXFGraphic(DXFEntity):
    ...
    :class:`~dxfpy.entities.dxfentity.DXFEntity`. These entities resides in
    ...
    def unlink_from_layout(self) -> None:
        ...
        :meth:`~dxfpy.layouts.BaseLayout.unlink_entity` method of the associated
    ...
    def transform(self, m: Matrix44) -> Self:
        ...
        Args:
             m: 4x4 transformation matrix (:class:`dxfpy.math.Matrix44`)
        """

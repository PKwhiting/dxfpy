...
if TYPE_CHECKING:
    ...
    from dxfpy.entities import DXFEntity, Textstyle
...
SHX_FONTS = {
    ...
    # and therefore can not be included in dxfpy or the associated repository!
...
def build_system_font_cache() -> None:
    ...
    location in the cache directory of the users home directory "~/.cache/dxfpy" or the
...
def get_font_face(font_name: str, map_shx=True) -> FontFace:
    ...
    Args:
        font_name: raw font file name as stored in the
            :class:`~dxfpy.entities.Textstyle` entity
...
def build_font_manager_cache(path: pathlib.Path) -> None:
    ...
    if not cache_dir.exists():
        logger.warning(
            ...
            f"not be saved.\nSee also issue https://github.com/mozman/dxfpy/issues/923."
        )
        return
    try:
        path.write_text(s)
...
class AbstractFont:
    """The `dxfpy` font abstraction for text measurement and text path rendering."""
...
class ShapeFileFont(_CachedFont):
    ...
    done by the dxfpy.fonts.shapefile module. The given cap height and width factor are
...
class LibreCadFont(_CachedFont):
    ...
    done by the dxfpy.fonts.lff module. The given cap height and width factor are the
...
def make_font(
    ...
    .. note:: The font definition files are not included in `dxfpy`.

    Args:
        font_name: font file name as stored in the :class:`~dxfpy.entities.Textstyle`
...
def get_entity_font_face(entity: DXFEntity, doc: Optional[Drawing] = None) -> FontFace:
    ...
    :class:`~dxfpy.entities.Textstyle` table entries.

...
from dxfpy.lldxf import validator, const
from dxfpy.enums import (
    TextEntityAlignment,
    TextHAlign,
...
from dxfpy.math import Vec3, Vec2, UVec
from dxfpy.colors import rgb2int, RGB, int2rgb


if TYPE_CHECKING:
    from dxfpy.entities import Text, MText, DXFEntity
...
class TextLine:
    ...
    (e.g. :class:`~dxfpy.entities.Text`).

    Args:
        text: content string
        font: dxfpy font definition like :class:`~dxfpy.fonts.fonts.MonospaceFont`
            or :class:`~dxfpy.fonts.fonts.TrueTypeFont`
...
def plain_text(text: str) -> str:
    """Returns the plain text for :class:`~dxfpy.entities.Text`,
    :class:`~dxfpy.entities.Attrib` and :class:`~dxfpy.entities.Attdef` content.
    """
...
#     dxfpy.rgb2int((31,224,114)) = 2089074 (r,g,b) wrong!
#     dxfpy.rgb2int((114,224,31)) = 7528479 (b,g,r) reversed order is correct!
#
...
def is_text_vertical_stacked(text: DXFEntity) -> bool:
    """Returns ``True`` if the associated text :class:`~dxfpy.entities.Textstyle`
    is vertical stacked.
    """
...
class ParagraphProperties(NamedTuple):
    ...
    Args:
         ...
         align: :class:`~dxfpy.lldxf.const.MTextParagraphAlignment` enum
...
class MTextEditor:
    ...
    DXF attributes of the :class:`~dxfpy.entities.MText` entity.

    .. warning::

        The :class:`MTextEditor` assembles just the inline code, which has to be
        parsed and rendered by the target CAD application, `dxfpy` has no influence
        to that result.
    ...
    def font(self, name: str, bold: bool = False, italic: bool = False) -> MTextEditor:
        ...
        """
        # c0 = current codepage
        # The current implementation of dxfpy writes everything in one
    ...
    def scale_height(self, factor: float) -> MTextEditor:
        ...
        :class:`~dxfpy.entities.MText` entity as DXF attribute
        :class:`~dxfpy.entities.MText.dxf.char_height`.
...
def estimate_mtext_extents(mtext: MText) -> tuple[float, float]:
    ...
    This function is faster than the :func:`~dxfpy.tools.text_size.mtext_size`
...
def estimate_mtext_content_extents(
    ...
    """Estimate the width and height of the :class:`~dxfpy.entities.MText`
    content string. The result is very inaccurate if inline codes are used or
    line wrapping at the column border is involved!
    Column breaks ``\\N`` will be ignored.

    Args:
        content: the :class:`~dxfpy.entities.MText` content string
        font: font abstraction based on :class:`dxfpy.tools.fonts.AbstractFont`

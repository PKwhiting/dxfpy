...
class NumpyPath2d(NumpyShape2d):
    ...
    for the :class:`dxfpy.path.Path` class. Operations like transform and reverse are
    done inplace to utilize the `numpy` capabilities. This behavior is different from the
    :class:`dxfpy.path.Path` class!!!
    ...
    @property
    def start(self) -> Vec2:
        """Returns the start point as :class:`~dxfpy.math.Vec2` instance."""
        return Vec2(self._vertices[0])

    @property
    def end(self) -> Vec2:
        """Returns the end point as :class:`~dxfpy.math.Vec2` instance."""
    ...
    def to_path(self) -> Path:
        """Returns a new :class:`dxfpy.path.Path` instance."""
    ...
    # Construct paths as dxfpy.path.Path and convert them to NumpyPath2d.
...
def to_qpainter_path(paths: Iterable[NumpyPath2d]):
    ...
    from dxfpy.addons.xqt import QPainterPath, QPointF

...
from dxfpy.render import MeshVertexMerger, MeshTransformer, MeshBuilder
from dxfpy.math import Matrix44, Vec3, NULLVEC, BoundingBox
from . import entities
from .entities import Body, Lump, NONE_REF, Face, Shell


def mesh_from_body(body: Body, merge_lumps=True) -> list[MeshTransformer]:
    """Returns a list of :class:`~dxfpy.render.MeshTransformer` instances from
    ...
    .. note::
        ...
        required which `dxfpy` does not provide.
...
def flat_polygon_faces_from_lump(
    ...
    sequence of :class:`~dxfpy.math.Vec3` instances. Applies the transformation
    :class:`~dxfpy.math.Matrix44` `m` to all vertices if not ``None``.
...
def body_from_mesh(mesh: MeshBuilder, precision: int = 6) -> Body:
    """Returns a :term:`ACIS` :class:`~dxfpy.acis.entities.Body` entity from a
    :class:`~dxfpy.render.MeshBuilder` instance.

    This entity can be assigned to a :class:`~dxfpy.entities.Solid3d` DXF entity
    ...
    separated :class:`~dxfpy.acis.entities.Lump` node.
    If each mesh should get its own :class:`~dxfpy.acis.entities.Body` entity,
    separate the meshes beforehand by the method
    :meth:`~dxfpy.render.MeshBuilder.separate_meshes`.
...
def lump_from_mesh(mesh: MeshBuilder) -> Lump:
    """Returns a :class:`~dxfpy.acis.entities.Lump` entity from a
    :class:`~dxfpy.render.MeshBuilder` instance. The `mesh` has to be a single
    body or shell!
    """
    lump = Lump()
    shell = Shell()
...
def vertices_from_lump(lump: Lump, m: Matrix44 | None = None) -> list[Vec3]:
    ...
    Applies the transformation :class:`~dxfpy.math.Matrix44` `m` to all vertices if not
    ``None``.

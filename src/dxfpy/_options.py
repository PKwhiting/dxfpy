...
def config_files() -> list[Path]:
    ...
    #    "$XDG_CONFIG_HOME/dxfpy/dxfpy.ini" or
    #    "~/.config/dxfpy/dxfpy.ini"
    # 2. current working directory "./dxfpy.ini"
    # 3. config file specified by DXFPY_CONFIG_FILE

    paths = default_config_files()
    env_cfg = os.getenv("DXFPY_CONFIG_FILE", "")
    if env_cfg:
...
def load_config_files(paths: list[Path]) -> ConfigParser:
    ...
    for name, env_name in [
        ("TEST_FILES", "DXFPY_TEST_FILES"),
        ("DISABLE_C_EXT", "DXFPY_DISABLE_C_EXT"),
    ]:
...
class Options:
    ...
    def __init__(self) -> None:
        ...
        self._use_c_ext = False  # set dxfpy.acc.__init__!
        self.debug = False
    ...
    def write_file(self, filename: str = DXFPY_INI) -> None:
    ...
    def write_home_config(self):
        """Write current configuration into file "~/.config/dxfpy/dxfpy.ini" or
        "XDG_CONFIG_HOME/dxfpy/dxfpy.ini".
        """
        ...
        filename = str(home_path / DXFPY_INI)
        try:

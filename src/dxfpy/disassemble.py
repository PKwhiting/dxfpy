...
import abc
import math
from dxfpy.entities import DXFEntity, Insert, get_font_name

from dxfpy.lldxf import const
from dxfpy.enums import TextEntityAlignment
from dxfpy.math import Vec3, UCS, Z_AXIS, X_AXIS, BoundingBox
from dxfpy.path import Path, make_path, from_vertices, precise_bbox
from dxfpy.render import MeshBuilder, MeshVertexMerger, TraceBuilder
...
if TYPE_CHECKING:
    from dxfpy.entities import LWPolyline, Polyline, MText, Text
...
class Primitive(abc.ABC):
    ...
    def path(self) -> Optional[Path]:
        """:class:`~dxfpy.path.Path` representation or ``None``,
    ...
    def mesh(self) -> Optional[MeshBuilder]:
        """:class:`~dxfpy.render.mesh.MeshBuilder` representation or ``None``,
    ...
    def bbox(self, fast=False) -> BoundingBox:
        """Returns the :class:`~dxfpy.math.BoundingBox` of the path/mesh
...
def to_paths(primitives: Iterable[Primitive]) -> Iterable[Path]:
    """Yields all :class:`~dxfpy.path.Path` objects from the given
...
def to_meshes(primitives: Iterable[Primitive]) -> Iterable[MeshBuilder]:
    """Yields all :class:`~dxfpy.render.MeshBuilder` objects from the given

...
def transform_paths(paths: Iterable[Path], m: Matrix44) -> list[Path]:
    ...
    Args:
        ...
        m: transformation matrix of type :class:`~dxfpy.math.Matrix44`
...
def transform_paths_to_ocs(paths: Iterable[Path], ocs: OCS) -> list[Path]:
    ...
    Args:
        paths: iterable of :class:`Path` or :class:`Path2d` objects
        ocs: OCS transformation of type :class:`~dxfpy.math.OCS`
...
def bbox(paths: Iterable[Path], *, fast=False) -> BoundingBox:
    """Returns the :class:`~dxfpy.math.BoundingBox` for the given paths.
...
def precise_bbox(path: Path) -> BoundingBox:
    """Returns the precise :class:`~dxfpy.math.BoundingBox` for the given paths."""
...
def fit_paths_into_box(
    ...
    Args:
        paths: iterable of :class:`~dxfpy.path.Path` objects
...
def render_lwpolylines(
    ...
    *,
    ...
    :class:`~dxfpy.entities.LWPolyline` entities.
    ...
    Returns:
        created entities in an :class:`~dxfpy.query.EntityQuery` object
...
def render_hatches(
    ...
    *,
    ...
    """Render the given `paths` into `layout` as
    :class:`~dxfpy.entities.Hatch` entities.
    The `extrusion` vector is applied to all paths, all vertices are projected
    onto the plane normal to this extrusion vector. The default extrusion vector
    is the WCS z-axis. The plane elevation is the distance from the WCS origin
    to the start point of the first path.
...
def render_mpolygons(
    ...
    *,
    ...
    """Render the given `paths` into `layout` as
    :class:`~dxfpy.entities.MPolygon` entities. The MPOLYGON entity supports
...
def render_lines(
    ...
    *,
    ...
    """Render the given `paths` into `layout` as
    :class:`~dxfpy.entities.Line` entities.
...
def render_splines_and_polylines(
    ...
    *,
    ...
    """Render the given `paths` into `layout` as :class:`~dxfpy.entities.Spline`
    and 3D :class:`~dxfpy.entities.Polyline` entities.
...
def add_ellipse(
    ...
    `path`, use :meth:`~dxfpy.math.ConstructionEllipse.from_arc` constructor
    of class :class:`~dxfpy.math.ConstructionEllipse` to add circular arcs.
    ...
    Args:
        path: :class:`~dxfpy.path.Path` object
        ellipse: ellipse parameters as :class:`~dxfpy.math.ConstructionEllipse`
            object
...
def add_spline(path: Path, spline: BSpline, level=4, reset=True) -> None:
    ...
    Args:
        path: :class:`~dxfpy.path.Path` object
        spline: B-spline parameters as :class:`~dxfpy.math.BSpline` object

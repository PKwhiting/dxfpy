...
#
#   The command `dxfpy -V` must always work!
#
...
from dxfpy.lldxf.validator import is_dxf_file, is_binary_dxf_file, dxf_info
from dxfpy.dwginfo import dwg_file_info

if TYPE_CHECKING:
    from dxfpy.entities import DXFGraphic
    from dxfpy.addons.drawing.properties import Properties, LayerProperties
...
class Draw(Command):
    ...
    def run(args):
        try:
            ...
            from dxfpy.addons.drawing.config import Configuration, BackgroundPolicy
            from dxfpy.addons.drawing.file_output import (
                open_file,
        ...
        if args.out is not None:
            ...
            output_dir = pathlib.Path(tempfile.mkdtemp(prefix="dxfpy_draw"))
...
class Browse(Command):
    ...
    def run(args):
        try:
            ...
            print(str(e))
            sys.exit(1)
        from dxfpy.addons import browser
...
class BrowseAcisData(Command):
    ...
    def run(args):
        try:
            ...
            print(str(e))
            sys.exit(1)
        from dxfpy.addons.acisbrowser.browser import AcisStructureBrowser
...
class Strip(Command):
    ...
    @staticmethod
    def run(args):
        from dxfpy.tools.strip import strip
...
class Config(Command):
    ...
    def add_parser(subparsers):
        ...
        parser.add_argument(
            "--home",
            action="store_true",
            help="create config file 'dxfpy.ini' in the user home directory "
            "'~/.config/dxfpy', $XDG_CONFIG_HOME is supported if set",
        )
        parser.add_argument(
            "--reset",
            action="store_true",
            help="factory reset, delete default config files 'dxfpy.ini'",
        )
...
class Info(Command):
    ...
    def add_parser(subparsers):
        parser = subparsers.add_parser(
            ...
            "loaded by dxfpy, this may not represent the original "
    ...
    @staticmethod
    def run(args):
        from dxfpy.document import info
...
def export_hpgl2(filepath: Path, args) -> None:
    ...
    from dxfpy.addons.drawing.dxf import ColorMode
...
def set_app_icon(app):
    from dxfpy.addons.xqt import QtGui, QtCore

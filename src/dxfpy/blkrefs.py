...
from dxfpy.lldxf.types import POINTER_CODES
from dxfpy.protocols import referenced_blocks

if TYPE_CHECKING:
    from dxfpy.document import Drawing
    from dxfpy.lldxf.tags import Tags
    from dxfpy.entities import DXFEntity, BlockRecord
...
class BlockDefinitionIndex:
    """Index of all :class:`~dxfpy.entities.BlockRecord` entities representing
    real BLOCK definitions, excluding all :class:`~dxfpy.entities.BlockRecord`
    ...
    def block_records(self) -> Iterator[BlockRecord]:
        """Returns an iterator of all :class:`~dxfpy.entities.BlockRecord`
    ...
    def has_handle(self, handle: str) -> bool:
        """Returns ``True`` if a :class:`~dxfpy.entities.BlockRecord` for the
    ...
    def by_handle(self, handle: str) -> Optional[BlockRecord]:
        """Returns the :class:`~dxfpy.entities.BlockRecord` for the given block
        record handle or ``None``.
        """
        return self._handle_index.get(handle)

    def by_name(self, name: str) -> Optional[BlockRecord]:
        """Returns :class:`~dxfpy.entities.BlockRecord` for the given block name
        or ``None``.
        """
...
class BlockReferenceCounter:
    ...
    MLEADERSTYLE) in a DXF document::
        ...
        import dxfpy
        from dxfpy.blkrefs import BlockReferenceCounter

        doc = dxfpy.readfile("your.dxf")
    ...
    def by_handle(self, handle: str) -> int:
        ...
        :class:`~dxfpy.entities.BlockRecord` handle.
        """
...
def count_references(
    ...
    from dxfpy.entities import XRecord, DXFTagStorage

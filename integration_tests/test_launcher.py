...
def test_version():
    result = subprocess.run(["dxfpy", "-V"], capture_output=True)
    assert result.returncode == 0
    assert result.stdout.startswith(b"dxfpy")


def test_audit_existing_file():
    result = subprocess.run(
        ["dxfpy", "audit", str(TEST_DATA / "POLI-ALL210_12.DXF")],
        capture_output=True,
    )
    assert result.returncode == 0
...
def test_audit_file_not_found():
    result = subprocess.run(
        ["dxfpy", "audit", "nofile.dxf"], capture_output=True
    )
    assert result.returncode == 0
